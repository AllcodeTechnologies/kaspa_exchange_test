...
///1. You have to generate wallet from Mnemonic passed in the Body
const generateWalletFromMnemonic = async (req, res) => {
  const { mnemonic } = req.body;
  res.status(200).json({ status: true, message: "Complete The task" });
};

///2. Generate New/Fresh Wallet for Kaspa
const generateWallet = async (req, res, next) => {
  res.status(200).json({ status: true, message: "Complete The task" });
};

///3. use The Wallet generated in the Second Task , and send Kaspa To Another Kaspa Address
